      ...
      kyc(body, (err, data) => {
        if (err) {
          ...
          if (data === true) {
            ...
            SmartCaller.RequestPayment(result.returnValues[0], result.returnValues[1], result.returnValues[2], '0x43435f5054');
          }
        }
      });
    });
    ...
    listenEvent('RequestVerifyId', (result) => {
      ...
      storeID(idt, result.returnValues[0], (err, data) => {
        if (err) {
          ...
          SmartCaller.RequestVerifyId(result.returnValues[0], result.returnValues[1], result.returnValues[2], verifyID);
        }
      });
    });
  }
