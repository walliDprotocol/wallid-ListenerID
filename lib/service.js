...
function sendTx(functionName, args, gasLimit, nonce, callback) {
...
SmartCaller.prototype = {
  createNonce: function(callback) {
    web3.eth.getTransactionCount(address_wallet, function(err, count) {
      if (err) {
        ...
        // let nonce = web3.utils.numberToHex(count);
        callback(count);
      }
    });
  },
  RequestPayment: function (userAddress, idt, opid, sdkey, nonce) {
    console.log('nonce: ', nonce);
    console.log('RequestPayment smartCaller method');
    let args = [ userAddress, idt, opid, sdkey ];
    let gasLimit = 1000000;

    sendTx(econtract.methods.acceptedToken, args, gasLimit, nonce, function(err, result) {
      if (err) {
  ...
  RequestVerifyId: function (userAddress, idt, opid, verifyId, nonce) {
    console.log('RequestVerifyId smartCaller method');
    let args = [ userAddress, idt, opid, verifyId ];
    let gasLimit = 3000000;

    sendTx(econtract.methods.acceptedUserData, args, gasLimit, nonce, function(err, result) {
      if (err) {
